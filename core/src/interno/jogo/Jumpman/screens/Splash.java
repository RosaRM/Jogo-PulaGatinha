...
public class Splash implements Screen {
    ...
    public void show() {
        ...
        // Define a animação: começa com alpha (transparência) zero
        Tween.set(splash, SpriteAccessor.ALPHA).target(0).start(tweenManager);

        // Animação de fade in e fade out da imagem
        Tween.to(splash, SpriteAccessor.ALPHA, 2).target(1).repeatYoyo(1, 0.1f)  // Faz a transição de fade in, depois fade out
            .setCallback(new TweenCallback() {  // Após a animação, troca para a tela principal
                @Override
                public void onEvent(int type, BaseTween<?> source) {
                    ((Game) Gdx.app.getApplicationListener()).setScreen(new MainMenu());  // Troca para a tela de MainMenu
                }
            }).start(tweenManager);  // Inicia o gerenciador de animações
    }
    ...
    @Override
    public void hide() {
        // Método chamado ao ocultar a tela (não implementado)
    }
